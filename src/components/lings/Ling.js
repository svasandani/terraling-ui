...
function Ling({ groupId }) {
  ...
  useEffect(() => {
    fetch(
      process.env.REACT_APP_API + "groups/" + groupId + "/lings/" + lingId,
    ...
    <>
      ...
      {mapData.length > 0 ? (
        <iframe
          className="map-view"
          frameBorder="0"
          src={`https://www.google.com/maps/embed/v1/place?key=${process.env.REACT_APP_GKEY}&zoom=3&center=${mapData}&q=${mapData}`}
        />
      ) : (
        <h3 className="no-map-data">
          We don't have map data for {data.ling_name} yet.
        </h3>
      )}
      <h2>Description</h2>
      <div
        className="card ling-description"
        dangerouslySetInnerHTML={{ __html: data.ling_description }}
      />
      <h2>Compare</h2>
      <h2>Properties</h2>
      {data.ling_properties.length === 0 ? (
        ...
        <AlphaTable
          name={data.ling_name}
          sort={nameSort}
          data={data.ling_properties}
          columnMap={columnMap}
          link={(url, id) => {
            return "/groups/" + groupId + "/properties/" + id;
          }}
        />
      )}
    </>
